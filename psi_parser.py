...
class Tree:
    ...
    def build(cls, text: str) -> 'Tree':
        ...
        def expand_neg(node: Node):
            # ['!', Node(symbol=('2', 1), left=None, right=None), '|', Node(symbol=('4', 1), left=None, right=None)]
            # node = node #or self.root
            # print(node.is_leaf())
            op_neg = {'&':'|', ' & ':' | ', '|':'&', ' | ':' & '}
            if node.is_leaf():
                node.symbol = (node.symbol[0], -1*node.symbol[1])
                return node
            else:
                node.symbol = op_neg[node.symbol]
                return Node(symbol = node.symbol, left=expand_neg(node.left), right=expand_neg(node.right))
        ...
        for char in cls._tokenize(text):
            ...
            if char == ')':
                ...
                if '!' in stack:
                    stack.remove('!')
                    node = expand_neg(Node(symbol=op, left=left, right=right))
                    stack.append(node)
